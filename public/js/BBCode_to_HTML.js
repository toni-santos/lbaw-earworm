...
var BBCodeHTML = function () {
    ...
    var _getRegEx = function (str) {
        ...
        for (; i < nrmatches; i += 1) {
            ...
            if (tokens[token]) {
                ...
                replacement +=
                    preg_quote(str.substr(0, str.indexOf(matches[i]))) +
                    tokens[token];
    ...
    var _getTpls = function (str) {
        ...
        for (; i < nrmatches; i += 1) {
            ...
            if (tokens[token]) {
                replacement +=
                    str.substr(0, str.indexOf(matches[i])) + "$" + position;
                str = str.substr(str.indexOf(matches[i]) + matches[i].length);
            }
        }
    ...
    me.bbcodeToHtml = function (str) {
        ...
        for (; i < nrbbcmatches; i += 1) {
            str = str.toString().replace(bbcode_matches[i], html_tpls[i]);
        }
    ...
    me.htmlToBBCode = function (str) {
        ...
        for (; i < nrhtmlmatches; i += 1) {
            str = str.toString().replace(html_matches[i], bbcode_tpls[i]);
        }
...
export const bbcodeParser = new BBCodeHTML();
