...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        NEW.tsvectors = (
            ...
            setweight(to_tsvector('english', COALESCE((SELECT string_agg(Genre.name, ' ') AS genres FROM Genre JOIN ProductGenre ON Genre.id = ProductGenre.genre_id AND ProductGenre.product_id = NEW.id), '')), 'C') ||
...
CREATE TRIGGER product_genre_update
    AFTER INSERT OR UPDATE ON ProductGenre
    FOR EACH ROW
